...
describe("auth session helpers", () => {
  ...
  it("serializes generic pending intents in the signed cookie session", async () => {
    ...
    setPendingIntent(
      session,
      { kind: "rename-list", listId: "list-1", name: "Desk favorites" },
      "/library"
    );
  ...
  it("looks up the current user from the persisted auth token and expires old sessions", async () => {
    ...
    // Act
    const maybeUser = await resolveCurrentUserFromSession(sessionStore as never, session);
    const maybeExpiredUser = await resolveCurrentUserFromSession(sessionStore as never, session);

    // Assert
    expect(maybeUser).toMatchObject({ email: "peter@example.com", id: "user-1" });
    expect(maybeExpiredUser).toBeNull();
    expect(deleteMany).toHaveBeenCalledWith({ where: { token: "session-token" } });
  });
  ...
  it("creates database-backed sessions with tokens and expiry", async () => {
    ...
    expect(storedSession.expiresAt.toISOString()).toBe("2026-05-07T00:00:00.000Z");
  });
});
