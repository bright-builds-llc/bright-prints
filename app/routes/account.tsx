...
function describePendingIntent(
...
export async function loader({ request }: Route.LoaderArgs) {
  const { getAuthSession, getCurrentUserFromRequest } =
...
export async function handleAccountSubmission(
  ...
  try {
    ...
    deps.setFlashMessage(
      session,
      buildDefaultFlash(mode, replayResult.replayed),
    );
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const { createUserAccount, signInUserWithPassword } =
    await import("~/lib/auth/user.server");
  const { replayPendingAppIntent } =
    await import("~/lib/pending-intents.server");
    ...
    replayPendingIntentAfterAuth: (options: {
      ...
      replayPendingIntentAfterAuth({
        ...
        replayIntent: (pendingIntent: PendingIntent, userId: string) =>
          replayPendingAppIntent(getDb(), userId, pendingIntent),
      } as never),
    setAuthToken,
    setFlashMessage,
    signInUserWithPassword,
